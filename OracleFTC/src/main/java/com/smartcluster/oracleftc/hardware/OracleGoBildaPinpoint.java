...
@I2cDeviceType
@DeviceProperties(
        name = "goBILDA® Pinpoint Odometry Computer Oracle Driver",
        xmlTag = "OracleGoBildaPinpoint",
        description = "goBILDA® Pinpoint Odometry Computer (IMU Sensor Fusion for 2 Wheel Odometry)"
)
public class OracleGoBildaPinpoint extends I2cDeviceSynchDevice<I2cDeviceSynchSimple> {
    ...
    public OracleGoBildaPinpoint(I2cDeviceSynchSimple deviceClient, boolean deviceClientIsOwned) {
    ...
    public Pose2d setPose(Pose2d pose) {
        writeByteArray(Register.X_POSITION, (floatToByteArray((float) pose.getX()/25.4f, ByteOrder.LITTLE_ENDIAN)));
        writeByteArray(Register.Y_POSITION, (floatToByteArray((float) pose.getY()/25.4f, ByteOrder.LITTLE_ENDIAN)));
        writeByteArray(Register.H_ORIENTATION, (floatToByteArray((float) pose.heading.log(), ByteOrder.LITTLE_ENDIAN)));
        return pose;
    }
    ...
    public Pose2dDual<Time> getPose(){
        return new Pose2dDual<Time>(
                new Vector2dDual<Time>(
                        new DualNum<>(xPosition, xVelocity),
                        new DualNum<>(yPosition, yVelocity)
                ).div(25.4),
                Rotation2dDual.exp(new DualNum<>(hOrientation,hVelocity))
        );
    }
